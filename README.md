#  CS50x — Harvard Computer Science Journey

A documented journey through **Harvard University's CS50x: Introduction to Computer Science**, covering computer science fundamentals, algorithms, memory, data structures, Python, SQL, web development, and Flask.

This repository contains my **problem sets, projects, notes, and coding exercises** completed throughout the course.

##  Preview

![CS50x Preview](harvard.png)

## Course Progress

| Week | Topic | Status | Started | Finished |
|---|---|---|---|---|
| Week 0 | Scratch & Basics | ✅ Complete | — | July 2026 |
| Week 1 | C | ✅ Complete | July 2026 | August 2026 |
| Week 2 | Arrays | ✅ Complete | August 2026 | **20 Aug 2026** |
| Week 3 | Algorithms | ⬜ In Progress | — | — |
| Week 4 | Memory | ⬜ Not Started | — | — |
| Week 5 | Data Structures | ⬜ Not Started | — | — |
| Week 6 | Python | ⬜ Not Started | — | — |
| Week 7 | SQL | ⬜ Not Started | — | — |
| Week 8 | HTML, CSS & JavaScript | ⬜ Not Started | — | — |
| Week 9 | Flask | ⬜ Not Started | — | — |
| Week 10 | Final Project | ⬜ Not Started | — | — |

> **Started:** July 2026  
> **Current progress:** 3 / 11 stages completed  
> **Latest milestone:** Week 2 — Arrays completed on 20 August 2026

##  What I'm Learning

### Computer Science Fundamentals
- Computational thinking
- Problem solving
- Algorithms
- Data structures
- Memory and computer architecture

### Programming
- C
- Python
- SQL
- JavaScript

### Web Development
- HTML
- CSS
- JavaScript
- Flask
- Databases

## Technologies

### Programming

[![C](https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=black)](#)
[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](#)
[![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)](#)

### Web Development

[![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)](#)
[![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)](#)
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)](#)
[![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)](#)

### Tools

[![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)](#)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](#)

## Repository Structure

```text
CS50x/
│
├── week0/
├── week1/
├── week2/
├── week3/
├── week4/
├── week5/
├── week6/
├── week7/
├── week8/
├── week9/
└── final-project/